...
static void on_gallery_active_cb(void *ctx)
{
    (void)ctx;
    ESP_LOGI(TAG, "Gallery activated — auto-starting music playback");
    esp_err_t ret = esp_bt_audio_playback_play();
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Gallery auto-play failed: %s", esp_err_to_name(ret));
    }
}
#endif  /* CONFIG_EXAMPLE_BT_UI_ENABLE */
...
static esp_err_t append_mac_suffix_to_device_name(char *device_name, size_t device_name_size)
{
    ...
    /* Limit name part to avoid truncation: suffix "_XXYY" = 5 chars + null */
    if (device_name_size > 6) {
        written = snprintf(device_name, device_name_size, "%.*s_%02X%02X",
                          (int)(device_name_size - 6), default_name, mac[4], mac[5]);
    } else {
        written = snprintf(device_name, device_name_size, "%s", default_name);
    }
...
static void playback_metadata_proc(esp_bt_audio_event_playback_metadata_t *event_data)
{
    ...
    if (event_data->type == ESP_BT_AUDIO_PLAYBACK_METADATA_COVER_ART) {
        ...
        if (event_data->value != NULL && event_data->length > 0) {
            if (event_data->type == ESP_BT_AUDIO_PLAYBACK_METADATA_ALBUM) {
                /* Show album name on UI */
                bt_ui_update_album(ui, (const char *)event_data->value);
            }
            char *target = NULL;
...
void app_main()
{
    /* Enable debug logging for AVRCP cover art diagnostics */
    esp_log_level_set("BT_AUD_AVRC_CT", ESP_LOG_DEBUG);
    esp_log_level_set("BT_AUD_EXAMPLE", ESP_LOG_DEBUG);
    ...
    bt_ui_config_t ui_cfg = {
        ...
        .next_cb = on_next_cb,
        .prev_next_ctx = NULL,
        .gallery_active_cb = on_gallery_active_cb,
        .gallery_ctx = NULL,
    };
