...
typedef struct {
    ...
    void (*gallery_active_cb)(void *ctx);              /*!< Gallery page activated (auto-play music) */
    void *gallery_ctx;                                 /*!< Context passed to gallery_active_cb */
} bt_ui_config_t;

/** Tab indices in the tabview */
#define BT_UI_TAB_GALLERY  0
#define BT_UI_TAB_MUSIC    1
#define BT_UI_TAB_DIALER   2
...
/**
 * @brief  Update the album name on the media page.
 *
 * @param[in]  ui     UI handle from bt_ui_create().
 * @param[in]  album  Album name string.
 */
void bt_ui_update_album(bt_ui_t *ui, const char *album);

/**
 * @brief  Update lyrics on the media page (up to 5 visible lines).
 *
 * @param[in]  ui     UI handle from bt_ui_create().
 * @param[in]  lines  Array of lyric line strings.
 * @param[in]  count  Number of lines (0 to clear).
 */
...
/**
 * @brief  Set the album name label on the media page.
 *
 * @param[in]  media_root  Media root object returned by bt_ui_media_create().
 * @param[in]  album       Album name (NULL or empty to clear).
 */
void bt_ui_media_set_album(lv_obj_t *media_root, const char *album);

/**
 * @brief  Set lyrics lines on the media page (up to 5 lines).
 *         The middle line is highlighted as the currently active line.
 *
 * @param[in]  media_root  Media root object returned by bt_ui_media_create().
 * @param[in]  lines       Array of lyric line strings.
 * @param[in]  count       Number of lines in the array (0 to clear).
 */
void bt_ui_media_set_lyrics(lv_obj_t *media_root, const char *lines[], int count);

/**
 * @brief  Dynamically set the background color of the media page.
 *
 * @param[in]  media_root  Media root object returned by bt_ui_media_create().
...
/**
 * @brief  Create the photo gallery page with embedded images and navigation arrows.
 *
 *         Displays a set of embedded images one at a time with left/right arrows
 *         for browsing. Activated via swiping left from the media player page.
 *
 * @param[in]  parent  Parent LVGL object.
 *
 * @return
 *       - Gallery  root object on success
 *       - NULL     on failure
 */
lv_obj_t *bt_ui_gallery_create(lv_obj_t *parent);

/**
 * @brief  Show the next image in the gallery.
 *
 * @param[in]  gallery_root  Gallery root object returned by bt_ui_gallery_create().
 */
void bt_ui_gallery_next(lv_obj_t *gallery_root);

/**
 * @brief  Show the previous image in the gallery.
 *
 * @param[in]  gallery_root  Gallery root object returned by bt_ui_gallery_create().
 */
